...
import { Mail, ArrowRight, Instagram, ShoppingBag, Menu, X } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const animate = () => {
        ...
        if (bg) bg.style.transform = `translateY(${currentOffset}px) scale(${currentScale})`;
    ...
    <div className="min-h-screen bg-[#0B0B0B] text-[#FAF7F2] font-sans">
        ...
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          >
            <img
              src="/main-logo.png"
              alt="Nihon47 logo"
              className="h-12 w-auto object-contain transition-opacity duration-500 cursor-pointer"
            />
          </a>
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.nav
              ...
              className="md:hidden bg-[#0B0B0B]/95 border-t border-white/10 flex flex-col items-center py-6 space-y-4 text-lg font-light"
            >
              {[
                { id: "story", label: "Story" },
                { id: "philosophy", label: "Philosophy" },
                { id: "collection", label: "First Light" },
                { id: "newsletter", label: "Join", icon: <Mail className="w-4 h-4" /> },
              ].map((item, i) => (
                <motion.a
                  key={item.id}
                  href={`#${item.id}`}
      ...
      <section className="relative text-center h-[100vh] flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-white px-4">
          <h1 className="text-4xl md:text-5xl font-bold opacity-0 animate-fade-in [animation-delay:0.5s]">
            Wear the cities of Japan.
          </h1>
          <p className="text-lg md:text-xl text-gray-300 mt-6 opacity-0 animate-fade-in [animation-delay:1s]">
            Minimal streetwear inspired by the 47 prefectures — where culture meets precision.
          </p>
          <div className="flex justify-center gap-4 mt-10 opacity-0 animate-fade-in [animation-delay:1.5s]">
            <button
            ...
            <button
              ...
              className="bg-transparent border border-gray-400 text-gray-200 px-6 py-2 rounded-full hover:bg-gray-700 transition duration-300"
            >
              Read the Story →
            </button>
          </div>
        ...
        <div className="grid md:grid-cols-5 gap-10 items-start">
          <div className="md:col-span-3">
            <h2 className="text-2xl md:text-3xl font-semibold">
              A tribute to Japan. Designed with intention.
            </h2>
            <p className="mt-4 text-white/80 leading-relaxed max-w-2xl">
              From Tokyo’s neon pulse to Kyoto’s quiet temples, each design carries a story from one of Japan’s 47 prefectures.
              Nihon47 exists to turn those places into something you can wear. Simple. Cultural. Intentional.
            </p>
          ...
          <div className="md:col-span-2">
            <div className="rounded-3xl border border-white/10 p-6 bg-white/5 backdrop-blur">
              ...
              <p className="text-sm mt-4 font-medium">
                NIHON<span className="align-super text-xs">47</span> <span className="ml-2 text-xs opacity-80">日本</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-end justify-between gap-6">
          <div>
            ...
            <p className="mt-3 text-white/80 max-w-2xl">
              Before the noise. Before the rush. A five-piece capsule that captures Japan at dawn. Minimal, balanced, quietly powerful.
            </p>
          </div>
          <a
            href="#newsletter"
            className="hidden md:inline-flex items-center gap-2 text-sm border border-white/20 px-4 py-2 rounded-xl hover:border-white/40"
          >
        ...
        <div className="mx-auto max-w-6xl px-4 py-16 grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <p className="mt-3 text-white/80 max-w-md">
              Join the list for early access to First Light. No noise. Just calm drops and quiet stories.
            </p>
          </div>
          <form onSubmit={(e) => e.preventDefault()} className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="w-full px-4 py-3 rounded-2xl bg-white text-[#0B0B0B] placeholder-[#0B0B0B]/50 focus:outline-none"
            />
      ...
      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-6xl px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <p className="text-xs text-white/60">
            © {new Date().getFullYear()} Nihon47 — Wear the cities of Japan.
          </p>
          <div className="flex items-center gap-4 text-white/70">
            <a href="#" aria-label="Instagram" className="hover:opacity-80">
              <Instagram className="w-5 h-5" />
            </a>
            <a href="#" aria-label="Shop" className="hover:opacity-80">
  ...
  return (
    <div className="group relative rounded-3xl overflow-hidden border border-white/10 bg-white/[0.03]">
      <div className="aspect-[4/5] p-6 flex flex-col justify-between">
        <div
          className="absolute inset-0 opacity-20 group-hover:opacity-25 transition-opacity"
          style={{
        ...
        <div className="flex items-start justify-between">
          ...
          {badge && (
            <span className="text-[10px] uppercase tracking-widest px-2 py-1 rounded-full border border-white/20">
              {badge}
            </span>
          )}
        </div>
        <div className="flex-1 flex items-center justify-center">
          <div className="relative w-44 h-52 rounded-xl bg-[#111] border border-white/10 flex items-center justify-center">
            ...
            <div className="absolute -bottom-3 text-[10px] tracking-[0.25em] opacity-90">
              NIHON<span className="align-super text-[9px]">47</span> <span className="ml-1">日本</span>
            </div>
          </div>
        </div>
        <div>
